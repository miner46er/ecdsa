...
public class Shamaq {
    ...
    private ArrayList<ArrayList<Byte>> keys = new ArrayList<ArrayList<Byte>>();
    ...
    private Shamaq(ArrayList<Byte> key) {
        round_count = 8;
        block_size = 8;
        keys = generate_round_keys(key, round_count, block_size);
    }

    private ArrayList<ArrayList<Byte>> generate_round_keys(ArrayList<Byte> key, int round_count, int block_size) {
        int half_block_size = block_size / 2;
        
        ArrayList<Byte> key_data = new ArrayList<Byte>();

        // Temp key data generation
        for (int i = 0; i < 8*8; i++) {
            key_data.add((byte)i);
        }

        ArrayList<ArrayList<Byte>> keys = new ArrayList<ArrayList<Byte>>();

        for (int i = 0; i < round_count; i++) {
            keys.add(new ArrayList<>(key_data.subList(half_block_size * i, half_block_size * (i+1))));
        }

        // System.out.println("generate_round_keys " + keys);

        return keys;
    }

    private ArrayList<Byte> xor(ArrayList<Byte> a, ArrayList<Byte> b) {
        if (a.size() != b.size()) {
            System.out.println("kok gak sama ukurannya? " + a.size() + " " + b.size());
        }

        ArrayList<Byte> result = new ArrayList<>();

        for (int i = 0; i < a.size(); i++) {
            result.add((byte) (a.get(i) ^ b.get(i)));
        }
    ...
    private ArrayList<Byte> rot_r(ArrayList<Byte> a, int n) {
        n = n % a.size();
        ArrayList<Byte> res = new ArrayList<Byte>();
        res.addAll(a.subList(n, a.size()));
        res.addAll(a.subList(0, n));
        // System.out.println("rot " + res);
        return res;
    }

    private byte sub_byte(byte b) {
        int row = (b >> 4) & 0xf;
        int column = b & 0xf;
        int result = aes_sbox_bytes[row * 16 + column] & 0xf;
        byte res = (byte)result;

        // System.out.println("sub_byte " + row + " " + column + " " + res);

        return res;
    }

    private ArrayList<Byte> sub(ArrayList<Byte> a) {
        ArrayList<Byte> subbed_bytes = new ArrayList<>();
        for (byte i : a) {
            subbed_bytes.add(sub_byte(i));
        }
        // System.out.println("sub " + subbed_bytes);
        return subbed_bytes;
    }

    private ArrayList<Byte> reverse(ArrayList<Byte> block) {
        ArrayList<Byte> L = new ArrayList<>(block.subList(0, block_size / 2));
        ArrayList<Byte> R = new ArrayList<>(block.subList(block_size / 2, block.size()));
        R.addAll(L);
        // System.out.println("reverse " + R);
        return R;
    }

    private ArrayList<Byte> round(ArrayList<Byte> block, ArrayList<Byte> round_key) {
        ArrayList<Byte> L = new ArrayList<>(block.subList(0, block_size / 2));
        ArrayList<Byte> R = new ArrayList<>(block.subList(block_size / 2, block.size()));

        ArrayList<Byte> Rprime = xor(R, round_key);
        Rprime = sub(Rprime);
        Rprime = rot_r(Rprime, round_key.get(0));
    ...
    public ArrayList<Byte> encrypt_block(ArrayList<Byte> block) {
        for (int i = 0; i < round_count; i++) {
            // System.out.println("----------------"+i+"---------------");
            block = round(block, keys.get(i));
        }
    ...
    public ArrayList<Byte> decrypt_block(ArrayList<Byte> block) {
    ...
    public static void main(String[] args) {
        ...
        Shamaq test = new Shamaq(key);

        ArrayList<Byte> encrypted = test.encrypt_block(key);
        ArrayList<Byte> decrypted = test.decrypt_block(encrypted);
